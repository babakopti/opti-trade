from apply_loss_prevention import getLspPerf
...
for pers_off in range(1, 21):
    ...
    for std_coef in X:
        Y.append(
            getLspPerf(std_coef, pers_off, prtWtsHash, DF_FILE )
        )
...
perf_df.to_csv( 'analysis-results/lsp_performance.csv', index = False )
